generate.square.dist.matrix=function(dist.table,metadata,x) {
  ...
  #metadata = a dataframe corresponding to the metadata of your samples. Samples must be rows. Only include ALL the samples for which you want to include in your 16S analysis.
  
  #x = the column number that corresponds to the sample names column. Note that sample names in metadata and dist.table must match EXACTLY.
  ...
  rownames(dist.matrix)=metadata[,x]
  colnames(dist.matrix)=metadata[,x]
